...
\section{\texttt{init-swapchain.rs}}
	...
	It is the \inquotes{windowing system} (e.g. X Window System, MS Windows, Wayland) which is responsible for displaying the data stored in a swap chain, by providing us with a \inquotes{surface} abstraction onto which data is drawn. Note that the windowing system is separate from the core Vulkan API, but Vulkan provides official (Khronos Group maintained) extensions which   
